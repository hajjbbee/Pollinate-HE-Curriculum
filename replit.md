...
**Learning Needs & Neurodivergent Adaptations**: Comprehensive support for neurodivergent children accessible through an expandable section in Family Settings for each child. Families can configure learning needs profiles including: Focus/Attention (with 0-10 intensity scale, formerly ADHD/Attention Differences), Sensory sensitivities (with 0-10 intensity scale, sensory profile dropdown removed), Extended abilities grouping (Gifted and Twice Exceptional toggles), Dyslexia/Reading Differences (with 0-10 intensity scale), Dysgraphia/Writing Differences (with 0-10 intensity scale), Dyscalculia/Maths Differences (with 0-10 intensity scale), and Anxiety/Perfectionism (with 0-10 intensity). Each profile includes clear descriptions of how activities will be adapted (e.g., shorter lessons, movement breaks, fidget ideas for focus/attention; visual schedules, sensory supports for sensory sensitivities; oral options, audiobooks for dyslexia; typing options, reduced writing for dysgraphia; manipulatives, visual maths for dyscalculia). The database schema stores 15 learning needs fields per child: hasAdhd, adhdIntensity, hasAutism, autismIntensity, sensoryProfile (deprecated - database column kept for backward compatibility but field omitted from insert schema and no longer collected via UI or used in AI prompts), isGifted, is2e, hasDyslexia, dyslexiaIntensity, hasDysgraphia, dysgraphiaIntensity, hasDyscalculia, dyscalculiaIntensity, hasAnxiety, anxietyIntensity, isPerfectionist. UI implemented with Accordion component containing bordered sections for each learning need category, using Switch components for toggles and Slider components for intensity scales. All fields are optional and stored with proper default values (false for booleans, 0 for intensities). AI curriculum generation automatically adapts activities based on these profiles with specific accommodation instructions for each learning need (shorter lessons, movement breaks for ADHD; visual schedules for autism; oral/audiobooks for dyslexia; typing/alternative outputs for dysgraphia; manipulatives/visual maths for dyscalculia; choice boards for anxiety).
...
**Download This Week PDF Feature**: Beautiful, printable/tablet-friendly weekly curriculum PDFs accessible via dashboard header button. Each PDF (4-8 pages) includes: (1) Sage-green styled cover page with family theme, week number, date range, and mum-friendly note about offline usage; (2) Daily learning plans for each child showing activities with 3 confidence-boosting examples (Quick & Easy, Medium Adventure, Deep-Dive Rabbit Hole); (3) Materials & shopping list page highlighting free/recycled/household items in green-bordered section, separating low-cost items; (4) Local happenings page displaying relevant events from the week with dates, locations, and "why it fits" explanations; (5) Three journal & reflection pages with ruled lines and sketch boxes for capturing learning moments; (6) Narration & discussion prompts page with Charlotte Mason-style questions and space for responses. PDF generation implemented with PDFKit on backend (/api/curriculum/download-week endpoint), styled with sage-green borders (#8A9A5B), plenty of white space, and designed for both printing and tablet annotation. Frontend button shows loading state during generation and displays success toast with "ready to print or use on tablet" message.
